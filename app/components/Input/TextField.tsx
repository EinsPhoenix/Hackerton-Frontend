...
const TextFieldComponent = <L extends LibraryTypes, R extends LibraryTypes>(
  ...
  ref?: React.Ref<CustomTextInputRef | null>,
) => {
  const {
    backgroundColor,
    borderColor,
  ...
  const debouncedValue = debounceDelay && useDebounce(inputValue, debounceDelay)
  const alreadyTyped = useRef<boolean>(false)

  useEffect(() => {
    if (onChangeTextDebounce && debounceDelay) {
      if (alreadyTyped.current) {
        onChangeTextDebounce(debouncedValue || '')
      }
  ...
  const active = useDerivedValue(
    () => focused.value || (inputValue?.length || 0) > 0,
...
const ForwardedTextField = React.forwardRef<
  CustomTextInputRef,
  TextFieldProps<LibraryTypes, LibraryTypes>
>(TextFieldComponent)
