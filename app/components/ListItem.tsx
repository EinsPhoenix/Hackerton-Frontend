...
> extends TouchableOpacityProps {
  ...
  draggable?: boolean
  dragItemLeft?: IconProps<D>['icon']
  dragItemRight?: IconProps<D>['icon']
  dragItemLibrary?: D
  onRightSwipe?: () => void
  onLeftSwipe?: () => void
  /**
...
export function ListItem(
  props: ListItemProps<LibraryTypes, LibraryTypes, LibraryTypes>,
): React.ReactNode {
  const {
    bottomSeparator,
    children,
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_event, gestureState) => {
        ...
        if (gestureState.dx > sizing.threshold.normal && onRightSwipe) {
          onRightSwipe()
        }
        if (gestureState.dx < sizing.threshold.normal && onLeftSwipe) {
          onLeftSwipe()
        }
      },
  ...
  return (
    <Animated.View
      ...
      style={[containerStyles, { transform: [{ translateX: dragX }] }]}>
      <TouchableOpacity {...TouchableOpacityProps} style={touchableStyles}>
        {dragItemLeft && (
          <Icon
            icon={dragItemLeft}
            library={dragItemLibrary}
            size={35}
            containerStyle={dragItemLeftStyle}
            noColorChange
          />
        )}
        ...
        {dragItemRight && (
          <Icon
            ...
            library={dragItemLibrary}
            size={35}
            containerStyle={dragItemRightStyle}
          />
        )}
      </TouchableOpacity>
    </Animated.View>
...
const dragItemLeftStyle: StyleProp<ViewStyle> = {
  left: -99,
  ...dragItemStyle,
}

const dragItemRightStyle: StyleProp<ViewStyle> = {
  right: -99,
  ...dragItemStyle,
}
